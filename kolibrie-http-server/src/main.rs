...
use datalogmtl::stream::{ShapeIngester, RdfEvent};
use datalogmtl::parser::{parse_program, parse_data};
use datalogmtl::rdf_parser::{
    parse_rules as parse_dmtl_rules,
    parse_stream_shapes,
    parse_interval as parse_dmtl_interval,
};
...
fn handle_client(mut stream: TcpStream, sessions: Sessions, dmtl_sessions: DmtlSessions) {
    match read_http_request(&mut stream) {
        Ok(request) => {
            ...
            if request.method == "GET" && request.path.starts_with("/datalogmtl/events/") {
                let id = request.path["/datalogmtl/events/".len()..].to_string();
...
fn handle_request(request: &HttpRequest, sessions: &Sessions, dmtl_sessions: &DmtlSessions) -> String {
    ...
    if method == "POST" && path == "/datalogmtl/register" {
        return match request_body(&request.body) {
            Some(body) => handle_dmtl_register(body, dmtl_sessions),
            None => json_error_response("Request body is not valid UTF-8"),
        };
    }
...
fn handle_dmtl_push(body: &str, dmtl_sessions: &DmtlSessions) -> String {
    ...
    let triples = match (&mut session.shape_ingester, &req.stream_iri) {
        (Some(ingester), Some(iri)) => {
            ...
            // Emit the shape's EXPIRY facts for channels that went quiet before
            // evicting them, otherwise a stale source disappears silently.
            let expired: Vec<Triple> = ingester
                .expiry_facts(req.timestamp)
                .into_iter()
                .map(|(t, _)| t)
                .collect();
            ingester.evict_expired(req.timestamp);
            let mut out = expired;
            out.extend(ingester.process_event(&event).into_iter().map(|(t, _)| t));
            out
        }
