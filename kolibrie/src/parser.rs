...
mod tests{
    ...
    fn test_rspql() {
        ...
        let query = r#"REGISTER RSTREAM <http://out/stream> AS
SELECT *
    ...
    WINDOW :wind {
        ...
        match parse_combined_query(query){
            Ok((remaining, parsed_query)) => {
                ...
                //println!("Query: {}", parsed_query.sparql);
                // println!("Remaining input: '{}'", remaining.trim());
                // println!("Window iri {}", parsed_query.register_clause.clone().unwrap().query.window_clause.unwrap().window_iri);
                // println!("Window sliide {}", parsed_query.register_clause.clone().unwrap().query.window_clause.unwrap().window_spec.slide.unwrap());
                // println!("Window withdt {}", parsed_query.register_clause.clone().unwrap().query.window_clause.unwrap().window_spec.width.to_string());
                // println!("Window query {}", parsed_query.register_clause.clone().unwrap().query.where_clause);
