...
fn check_for_prefix_errors(lines: &[&str]) -> Option<String> {
    ...
    for (line_idx, line) in lines.iter().enumerate() {
        ...
        // Lines with prefix declarations
        if trimmed.to_uppercase().starts_with("PREFIX ") {
            ...
            if parts.len() < 2 {
                return Some(format!(
                    "\nMalformed PREFIX declaration at line {}:\n{}\n    \
                     Expected something like 'PREFIX ex: <http://...>'\n",
                    line_idx + 1,
                    line
                ));
            }
            ...
            if !prefix_candidate.contains(':') {
                return Some(format!(
                    "\nMissing colon in prefix declaration at line {}:\n{}\n    \
                     Found '{}', but expected something like 'ex:'\n",
                    line_idx + 1,
                    line,
                    prefix_candidate
                ));
            ...
            // If it does contain ":", parse out the prefix portion
            if let Some(idx) = prefix_candidate.find(':') {
                let just_prefix = &prefix_candidate[..idx]; // e.g. "ex" from "ex:"
                if just_prefix.is_empty() {
                    return Some(format!(
                        "\nEmpty prefix name at line {}:\n{}\n    \
                         Found '{}', but prefix name is empty!\n",
                        line_idx + 1,
                        line,
                        prefix_candidate
                    ));
...
fn check_for_mismatched_braces(lines: &[&str]) -> Option<String> {
    ...
    let total_open_braces = input.chars().filter(|c| *c == '{').count();
    let total_close_braces = input.chars().filter(|c| *c == '}').count();
    ...
    if total_open_braces != total_close_braces {
        ...
        for (line_idx, line) in lines.iter().enumerate() {
            let line_open = line.chars().filter(|c| *c == '{').count();
            let line_close = line.chars().filter(|c| *c == '}').count();
...
fn check_for_select_missing_where(input: &str) -> Option<String> {
    ...
    if lower.contains("select") && !lower.contains("where") {
        return Some(
            "\nFound a 'SELECT' keyword but no corresponding 'WHERE' clause.\n\
             (Could be a false positive if 'SELECT' is in a string/comment.)\n"
            .to_string()
        );
    }
    None
...
fn check_for_incomplete_triples(lines: &[&str]) -> Option<String> {
    ...
    for (idx, line) in lines.iter().enumerate() {
        ...
        // If there's fewer than 3 tokens, and no ending '.' or ';'
